...
{
	let wgpu_framework = WebGpuFramework::init();
    let _ = wgpu_framework.run::<T>();
}
...
impl WebGpuFramework
{
	...
	fn init() -> Self
	{
		...
		let window_size = LogicalSize::new(WebGpuFramework::WINDOW_WIDTH, WebGpuFramework::WINDOW_HEIGHT);
		let window = Arc::new(
			WindowBuilder::new()
				.with_title(WebGpuFramework::WINDOW_TITLE)
    ...
    fn run<T: FrameworkLoop + InputEvent>(
        mut self,
